...
4. Serves **inclusion proofs** on demand (per-submission via the inclusion receipt and per-query via the signed `audit_query_response` envelope). **Consistency-proof** verification between two checkpoints is pinned by [`ink-merkle-consistency.md`](ink-merkle-consistency.md) (`verifyConsistencyProof`, shipped in 0.5.0), and the reference Go witness serves proofs at `GET /consistency`; that serving surface is not yet a normative endpoint of this document. Consistency verification, together with external `tlog-witness` cosigners (§7.0), is the mitigation against split-view attacks.
