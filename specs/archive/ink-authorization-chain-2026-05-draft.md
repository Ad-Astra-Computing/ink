> **Superseded.** This 2026-05-24 draft is superseded by
> [`../ink-authorization-chain.md`](../ink-authorization-chain.md). It is
> retained for provenance only and is not normative. Its schemas (the envelope
> `delegationProof` field, the `permissions` arrays, `maxAutonomyTier`,
> `allowedTransports`, the `tulpa/0.2` embedding and the per-hop
> `revocationEndpoint`) do not describe the current extension.
