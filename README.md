...
**Status: experimental.** `ink/0.2` is the current defined wire version for the intent envelope only; every other wire object stays `ink/0.1`. Wire formats, trust semantics and APIs may change without backward-compatible migration before v1.0. On npm, `latest` is `0.18.0`[^ck] and `next` is `0.18.0`[^ck]; senders still emit `ink/0.1` by default unless explicitly configured.
