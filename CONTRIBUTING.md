...
2. Run all six CI checks, every one must pass before requesting review:
   ...
   A change to `package-lock.json` or `go/go.sum` has one more: `nix flake check` at the root, and in `go/` for the Go side. It builds the packages, which is where `npmDepsHash` in `flake.nix` and `vendorHash` in `go/flake.nix` are verified. When one is stale the check fails. On a cold store nix prints the hash it got; when the old dependency derivation is still in your store the failure comes from the build instead, so recompute with `nix run --inputs-from . nixpkgs#prefetch-npm-deps -- package-lock.json` (or set the hash to `lib.fakeHash` and read the value back). An install through npm alone never checks either.

   Only a release pull request can fail `check:release-pin`, since only a release pull request bumps the version. A release pull request also bumps the Go pin and the `Version` constant in `go/internal/cli/cli.go` alongside it, which `npm run check:release-parity` gates. [RELEASING.md](RELEASING.md) covers why the two implementations publish together. When it does, update `governance/releases/npm-dist-tags.json` in the same commit and rerun `npm run check:facts` to pick up the documents that quote a dist-tag. The publish workflow runs a stricter form of the same check and refuses to publish without it.
