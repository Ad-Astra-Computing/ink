...
- The dual-implementation agreement holds. A 1.0 wire claim is only credible while the TypeScript reference and the Go verifier agree on the whole base corpus. Losing that agreement is a release blocker. It also has to be an agreement an adopter can install: the two implementations publish at the same version from the same commit, under the rules in [RELEASING.md](RELEASING.md), and `npm run check:release-parity` gates that on every commit.
