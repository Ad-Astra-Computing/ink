...
- The soak-dependent evidence in that readiness record has been written and reviewed. The exit criteria there are a fixed bar and are normative: a production integrator runs the candidate against real inbound and outbound traffic for a defined window of evidenced days with no signature, namespace or encrypted-path regressions, the evidenced-behavior criterion holds on inbound blocks, the nightly shadow census and the daily attested synthetic exchange, and the conformance corpus and `go-conformance` job stay green on the release commit across a recorded weekly cadence. An unevidenced day is not a soak day, and absence of evidence is `INSUFFICIENT_DATA` rather than a pass. Meeting the bar is necessary. It is not automatically sufficient. The lead maintainer still gates the promotion.
