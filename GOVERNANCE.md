...
- The soak-dependent evidence in that readiness record has been written and reviewed. The exit criteria there are a fixed bar: a production integrator runs the candidate against real inbound and outbound traffic for the full soak window with no signature, namespace or encrypted-path regressions, no rise in rejection or transport-auth-failure rates attributable to the release and a green corpus and `go-conformance` job for the duration. Meeting the bar is necessary. It is not automatically sufficient. The lead maintainer still gates the promotion.
